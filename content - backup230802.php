...
if ($_GET['module']=='home'){
	echo "<h1>DASHBOARD</h1>
			<h3>JUMLAH DOKIM WNA YANG DIARSIPKAN KEDALAM APLIKASI PANDAWA 7 HARI TERAKHIR</h3>";
	
	
	echo"<iframe src='app/chart1.php' height='400px' width='700px'></iframe>
			<br><br><hr color=#265180><br>
	 
			<h3>JUMLAH DOKIM WNA YANG DISERAHKAN DALAM 7 HARI TERAKHIR</h3>
...
elseif ($_GET['module']=='hasilpencarian'){
	...
	if($_POST['kategori']=='nomor_permohonan'){
		$cari   = mysqli_query($conn,"SELECT * FROM arsip_dokim WHERE nomor_permohonan = '$_POST[kata]'");
	...
	//HALAMAN CARI DOKUMEN DENGAN NAMA	
		...
		$cari   = mysqli_query($conn,"SELECT * FROM dokim_wna WHERE nama LIKE '%$_POST[kata]%'");
		$jumlah = mysqli_num_rows($cari);
		...
		if ($jumlah == 1){
			...
			if ($jmlst > 0){
				...
				<tbody>";
				$tampilMas = mysqli_query($conn,"SELECT a.*, b.* FROM dokim_wna a LEFT JOIN arsip_dokim b on a.nomor_permohonan = b.nomor_permohonan WHERE a.nama LIKE '%$_POST[kata]%'");
				$mas = mysqli_fetch_array($tampilMas);
					echo"<tr align='center'>
					<td>$mas[nomor_permohonan]</td>
					<td>$mas[nomor_dokim]</td>
		...
		//HALAMAN JIKA DITEMUKAN LEBIH DARI SATU DOKUMEN DENGAN NAMA		
				...
				<tbody>";
				$tampilMas = mysqli_query($conn,"SELECT a.*, b.* FROM dokim_wna a LEFT JOIN arsip_dokim b on a.nomor_permohonan = b.nomor_permohonan WHERE a.nama LIKE '%$_POST[kata]%' ORDER by b.status");
...
elseif ($_GET['module']=='dokimsimpan'){
	...
	$sebelum        = $page - 1;
	$setelah        = $page + 1;
	$result = mysqli_query($conn,"SELECT * FROM arsip_dokim where status='Aktif'");
	$total = mysqli_num_rows($result);
	$pages = ceil($total/$halaman);
	$tampilMas = mysqli_query($conn,"SELECT a.*, b.* FROM arsip_dokim a LEFT JOIN dokim_wna b on a.nomor_permohonan = b.nomor_permohonan
									where status='Aktif' order by a.nomor_rak LIMIT $mulai, $halaman");
	$no = $mulai+1;
                while($mas = mysqli_fetch_array($tampilMas)){
	echo "<tr align='center'>
			  <td>$no</td>
...
elseif ($_GET['module']=='detildokimsimpan'){
	...
	echo"   <tr><td colspan='2'><hr></td></tr>
			...
			<tr><td>Ganti Lokasi Rak</td><td><select name='nomor_rak' REQUIRED>";
												$cekrak = mysqli_query($conn,"SELECT * FROM rak where nomor_rak != '$d[nomor_rak]' ORDER BY nomor_rak");
												while ($rak = mysqli_fetch_array($cekrak)){
													$kuota = mysqli_query($conn,"SELECT kuota FROM rak where nomor_rak='$rak[nomor_rak]'");
													while ($k = mysqli_fetch_array($kuota)){;
														$terpakai = mysqli_query($conn, "SELECT COUNT(nomor_rak) as terpakai FROM arsip_dokim where nomor_rak='$rak[nomor_rak]' and status='aktif'");
...
elseif ($_GET['module']=='dokimserah'){
	...
	$sebelum        = $page - 1;
	$setelah        = $page + 1;
	$result = mysqli_query($conn,"SELECT * FROM arsip_dokim where status='Serah'");
	$total = mysqli_num_rows($result);
	$pages = ceil($total/$halaman);
	$tampilMas = mysqli_query($conn,"SELECT a.*, b.* FROM arsip_dokim a LEFT JOIN dokim_wna b on a.nomor_permohonan = b.nomor_permohonan
									where status='Serah' order by a.tanggal_serah DESC LIMIT $mulai, $halaman");
	$no = $mulai+1;
                while($mas = mysqli_fetch_array($tampilMas)){
	echo "<tr align='center'>
			  <td>$no</td>
...
elseif ($_GET['module']=='hasilpencariandokimserah'){
	...
	if($_POST['kategori']=='nomor_permohonan'){
	$status = mysqli_query($conn,"SELECT * FROM arsip_dokim WHERE nomor_permohonan = '$_POST[kata]' AND status='Serah'");
	$jmlst = mysqli_num_rows($status);
	...
	//PENCARIAN DENGAN NAMA
		...
		$status = mysqli_query($conn,"SELECT a.*, b.* FROM dokim_wna a LEFT JOIN arsip_dokim b on a.nomor_permohonan = b.nomor_permohonan WHERE a.nama LIKE '%$_POST[kata]%' AND b.status='Serah'");
		$jmlst = mysqli_num_rows($status);
...
elseif ($_GET['module']=='cekkirimarsip'){
	...
	$status = mysqli_query($conn,"SELECT a.*, b.* FROM arsip_dokim a
												 LEFT JOIN dokim_wna b ON a.nomor_permohonan=b.nomor_permohonan
												 WHERE a.status='Serah' AND (a.tanggal_serah>='$_POST[tanggal_awal]' AND a.tanggal_serah<='$_POST[tanggal_akhir]')");
	$st = mysqli_num_rows($status);
	$no=1;
	echo"<tr><td colspan='8'><i>Jumlah Dokim Berstatus Sudah Serah Pada Tanggal $_POST[tanggal_awal] s/d $_POST[tanggal_akhir]</i> : <b>$st</b></td></tr>";
