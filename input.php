...
if ($module=='dokumen' AND $act=='input'){
	...
	if ($jumlah > 0){
		...
		if (mysqli_query($conn, $sql)) { 
			header('location:?module=arsip&act=input&nip='.$nip.'&nomor_permohonan='.$nomor_permohonan.'&nomor_rak='.$nomor_rak.'&nama='.$nama.'&no_hp='.$no_hp.'&nomor_paspor='.$nomor_paspor.'&jenis_paspor='.$jenis_paspor.'&email='.$email.'&seksi='.$seksi.'&jenis_pelayanan='.$jenis_pelayanan); 
		}else{
			echo "Error: " . $sql . "
			" . mysqli_error($conn);
		}
...
elseif ($module=='arsip' AND $act=='input'){
	 ...
	 $jenis_pelayanan=$_GET['jenis_pelayanan'];
	...
	if (mysqli_query($conn, $sql)) { 
		header('location:?module=logarsip&act=berhasil&nip='.$nip.'&nomor_permohonan='.$nomor_permohonan.'&nomor_rak='.$nomor_rak.'&nama='.$nama.'&no_hp='.$no_hp.'&nomor_paspor='.$nomor_paspor.'&jenis_paspor='.$jenis_paspor.'&email='.$email.'&seksi='.$seksi.'&jenis_pelayanan='.$jenis_pelayanan);
	}else{
		echo "Error: " . $sql . "
" . 	mysqli_error($conn);
	}
...
elseif ($module=='logarsip' AND $act=='berhasil'){
	...
	if (mysqli_query($conn, $sql)) { 
		header('location:?module=arsip&act=kirimwa&nomor_permohonan='.$nomor_permohonan.'&nama='.$nama.'&no_hp='.$no_hp.'&nomor_paspor='.$nomor_paspor.'&jenis_paspor='.$jenis_paspor.'&email='.$email.'&seksi='.$seksi.'&nomor_rak='.$nomor_rak.'&jenis_pelayanan='.$jenis_pelayanan);
	}else{
		echo "Error: " . $sql . "
		" . mysqli_error($conn);
	}
...
elseif ($module=='arsip' AND $act=='kirimwa'){
	...
	if($layanan=='Reguler'){
		...
		curl_close($curl);
		echo $response;
	}elseif($layanan=='Percepatan'){
		$token = "$s[token]";
		$pesan = "$s[pesan_percepatan]";
		$target = "$no_hp|$nama|$nama|$nomor_permohonan|$ket|$nomor_paspor";
...
elseif ($module=='arsip' AND $act=='hapus'){
	...
	$log = "INSERT INTO log (tanggal,
					...
					VALUES	('$tanggal',
							 '$nip',
							 'Berhasil Menghapus Arsip Paspor dengan Nomor Permohonan $d[nomor_permohonan] atas nama $d[nama] dari Rak $d[nomor_rak]')";
